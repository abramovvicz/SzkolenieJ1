...
public class Main {
    public static void main(String[] args) throws IllegalAccessException, InstantiationException, InvocationTargetException {
        Class clazz = Animal.class; // statyczny dostęp do klasy przez refleksję - to nie jest obiektem teraz
        ...
        for (Method m : metods) {
            if (m.isAnnotationPresent(RunIt.class)) {   //sprawdzenie gdzie sa adnotacje
                ...
                System.out.println(m.getAnnotation(RunIt.class).name()); //m.getAnnotation(RunIt.class). - daje nam obiekt klasy RunIt
                m.invoke(o);
            }
        }
    }
